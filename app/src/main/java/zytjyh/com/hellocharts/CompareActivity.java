...
/**
 * Created by ZYTJYH on 13/3/2018.
 */

public class CompareActivity extends AppCompatActivity {
    static private int numberOfLines = 4;
    static private int numberOfPoints = 12;
    static float[][] temp=new float[numberOfLines][numberOfPoints];
    static  float[][] randomNumbersTab = new float[numberOfLines][numberOfPoints];
    static float[][] resultArray= new float[numberOfLines][numberOfPoints];
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        numberOfPoints=getIntent().getIntExtra("numOfOperation",0);
        if(getIntent().getStringArrayListExtra("FIFO").size()!=0)
        {
            for(int i=0;i<numberOfPoints;i++)
            {
                resultArray[0][i]=(float)(Float.valueOf(getIntent().getStringArrayListExtra("FIFO").get(i)));
            }
            Log.e("s",""+resultArray[0][0]);
        }
        if(getIntent().getStringArrayListExtra("LRU").size()!=0)
        {
            for(int i=0;i<numberOfPoints;i++)
            {
        ...
        if(getIntent().getStringArrayListExtra("LFU").size()!=0)
        {
            for(int i=0;i<numberOfPoints;i++)
            {
        ...
        if(getIntent().getStringArrayListExtra("OTP").size()!=0)
        {
            for(int i=0;i<numberOfPoints;i++)
            {
    ...
    public static class PlaceholderFragment extends Fragment {
        ...
        private void initValues(int flag) {
            ...
            if(flag==0) {
                ...
                for (int i = 0; i < numberOfLines; ++i) {
                    for (int j = 0; j < numberOfPoints; ++j) {
                        randomNumbersTab[i][j] = resultArray[i][j];
                    }
                }
            }
        }
        ...
        private void generateData() {
            ...
            for (int i = 0; i < numberOfLines; ++i) {
                ...
                switch (i){
                    case 0:
                        line.setColor(Color.parseColor(getResources().getString(R.string.FIFOColor)));
                        break;
                    case 1:
                        line.setColor(Color.parseColor(getResources().getString(R.string.LRUColor)));
                        break;
                    case 2:
                        line.setColor(Color.parseColor(getResources().getString(R.string.LFUColor)));
                        break;
                    case 3:
                        line.setColor(Color.parseColor(getResources().getString(R.string.OTPColor)));
                        break;
                }
        ...
        private void prepareDataAnimation() {
            ...
            for (Line line : data.getLines()) {
                for (PointValue value : line.getValues()) {
                    ...
                    value.setTarget(value.getX(), randomNumbersTab[i][j]);
                    j++;
